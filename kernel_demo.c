...
int send_usrMsg(char *message, int pid) {
    struct sk_buff *skb;
    struct nlmsghdr *nlh;
    int len = NLMSG_SPACE(MAX_MSG_SIZE);  //返回不小于NLMSG(len)的且字节对齐的最小数值。
    int ret_val;

    if (!message || !nl_sk) { return -1; }
...
//    memcpy(NLMSG_DATA(nlh), message, len);
    ...
    // 发送数据
    ret_val = netlink_unicast(nl_sk, skb, pid, MSG_DONTWAIT); //pid -- port id
    return ret_val;
}
...
void recv_nlmsg(struct sk_buff *skb) {
    ...
    pid = nlh->nlmsg_pid;
    send_usrMsg("I am from kernel!", pid);
}

struct netlink_kernel_cfg nl_kernel_cfg = {
        .groups = 0,
        .flags = 0,
        .input = recv_nlmsg, //设置收到消息时的回调
        .cb_mutex = NULL,
        .bind = NULL,
        .compare = NULL,
};
